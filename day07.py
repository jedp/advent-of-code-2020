...
ruleP = re.compile("^(?P<variety>.*?) bags contain (?P<contents>.*?).$")
containedP = re.compile("(?P<quantity>\d+) (?P<color>.*?) bags?")
noContentsP = re.compile("no other bags")
...
class Variety:
    ...
    def __str__(self):
        s = "%s contains: " % (self.color)
        ca = []
        if len(self.contents) == 0:
            ca.append("nothing")
        else:
            for c in self.contents:
                ca.append("%s x %d" % (c.color, self.contents[c]))
        s += ", ".join(ca) + "."
        s += " (contained by %s)" % (map(lambda x: x.color, self.containedBy))
        return s

    def contains(self, variety, quantity):
        # print "%s adding %s x %d" % (self.color, variety.color, quantity)
        self.contents[variety] = quantity
        variety.isContainedBy(self)

    def isContainedBy(self, other):
        self.containedBy.add(other)
...
class Bags:
    ...
    def __str__(self):
        s = ""
        for k in self.varieties:
            s += str(self.varieties[k]) + "\n"
        return s

    def rule(self, rule):
        matchD = ruleP.match(rule).groupdict()
        variety = self.get(matchD['variety'])

        for content in matchD['contents'].split(", "):
            if noContentsP.match(content):
                continue

            contentD = containedP.match(content).groupdict()
            contentBag = self.get(contentD['color'])
            quantity = int(contentD['quantity'])

            variety.contains(contentBag, quantity)
    ...
    def get(self, color):
        if color not in self.varieties:
            self.varieties[color] = Variety(color)

        return self.varieties[color]

    def ultimatelyCanContain(self, color):
        v = self.varieties[color]

        containers = set()
        for containedBy in v.containedBy:
            if containedBy.color in containers:
                continue

            containers.add(containedBy.color)
            containers = containers.union(self.ultimatelyCanContain(containedBy.color))

        return containers

    def sumOfBags(self, color):
        x = 1
        v = self.varieties[color]
        for c in v.contents:
            x += v.contents[c] * self.sumOfBags(c.color)

        return x

def test():
    bags = Bags()
    bags.ruleset(test_rules)

    assert(len(bags.ultimatelyCanContain('shiny gold')) == 4)

    print "shiny gold in: %s" % (bags.ultimatelyCanContain('shiny gold'))
...
def solve():
    ...
    color = 'shiny gold'
    containers = bags.ultimatelyCanContain(color)
    print "%d bags can contain %s: %s." % (
            len(containers),
            color,
            ", ".join(containers))
...
def test2():
    bags = Bags()
    bags.ruleset(test_rules2)

    color = 'shiny gold'
    contained = bags.sumOfBags(color) - 1
    print "%s contains %d other bags" % (color, contained)
    assert(contained == 126)
